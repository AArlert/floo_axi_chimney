...
#(
  ...
  // [本地补丁 P-001] VCS 2018 无法在端口位宽处折叠 floo_iomsb(RouteCfg.NumRoutes)
  // （struct 参数成员选择+常量函数调用报 TCF-ICTC）；localparam 中转后可折叠。
  localparam int unsigned RouteTblMsb       = floo_iomsb(RouteCfg.NumRoutes)
) (
  input  logic clk_i,
  input  logic rst_ni,
  ...
  input  route_t [RouteTblMsb:0] route_table_i,  // [本地补丁 P-001]
  /// Output links to NoC
