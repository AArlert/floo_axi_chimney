...
def tb_delivered(comp):
    """交付状态机械推导：tb/ 下是否存在 <组件>.sv（递归）；非单文件条目（如 "(全环境)"）返回 None。
    DUT 是 vendor 只读快照，本项目的交付物 = TB 组件。"""
    if not re.fullmatch(r"\w+", comp):
        return None
    return any(TB_DIR.rglob(f"{comp}.sv"))


def fm_stats(fm_rows, tp_pass):
    """feature-matrix 派生统计：交付由 tb 文件现算，验证由 testplan 现算——均不落盘。"""
    out = {}
    for r in fm_rows:
        d = out.setdefault(r.get("里程碑", "?"), {"total": 0, "rtl_total": 0, "deliv": 0, "verif": 0})
        d["total"] += 1
        dv = tb_delivered(r.get("组件", ""))
        if dv is not None:
            d["rtl_total"] += 1
            d["deliv"] += dv
...
def cmd_handover():
    ...
    print("== floo_axi_chimney DV 接手摘要 ==")
    ...
    print("\n-- feature-matrix（交付=tb 文件现算；验证=testplan 现算；均不落盘）--")
    tp_pass = testplan_pass_ids(tp_rows)
    for ms, d in sorted(fm_stats(fm_rows, tp_pass).items()):
        print(f"  {ms}: TB 组件交付{d['deliv']}/{d['rtl_total']}  验证✅{d['verif']}/{d['total']}")
...
def cmd_next():
    ...
    for r in parse_table(BUGS):
        ...
        if st == "OPEN":
            ...
            elif "dut" in owner.lower() or "rtl" in owner.lower():
                acts.append((1, f"{bid} OPEN（疑似 DUT/vendor RTL）→ 派 rev 确认：真 RTL 缺陷则记录+绕行（vendor 只读，可上报上游）"))
            else:
                acts.append((1, f"{bid} OPEN → 判归属：疑似 TB→用户修复（rev 复核）/ 疑似 spec→rev 仲裁"))
        elif st == "FIXING":
            acts.append((1, f"{bid} FIXING → 待修复者回填根因+修复 commit，置 FIX_READY"))
        elif st == "FIX_READY":
            acts.append((1, f"{bid} FIX_READY → 复验（登记的 TEST+SEED 复跑；关单人≠修复人）"))
        elif st == "VERIFYING":
            acts.append((1, f"{bid} VERIFYING → 复跑后用 evidence.py --bug {bid} 回填关单"))
    ...
    for r in cur_tp:
        if "❌" in r.get("状态", ""):
            acts.append((1, f"testplan {r.get('ID')} ❌ → 用户先自查激励/checker，仍疑似 DUT 则登 bugs.md"))
    mods = {}
    for r in cur_fm:
        mods.setdefault(r.get("组件", "?"), []).append(r)
    for mod, rows in mods.items():
        deliv = tb_delivered(mod)  # None = 非单文件条目（如 "(全环境)"），无交付概念
        ids = [r.get("编号", "?") for r in rows]
        unverif = sorted({s for r in rows for s in linked_scenes(r) if s not in tp_pass})
        if deliv is False:
            acts.append((2, f"组件 {mod} 未交付（条目 {' '.join(ids)}）→ 主会话出骨架，用户填核心逻辑，rev 审查"))
        elif unverif:
            who = "用户写场景/跑仿真" if deliv else "按条目性质推进（回归/覆盖率类走 make regress/cov）"
            acts.append((2, f"{mod} 场景 {' '.join(unverif)} 未 ✅ → {who}"))

    # 3) 里程碑完成判据（CLAUDE.md §4.1 三条硬条件；交付由 rtl 文件现算）
    if cur_fm and cur_tp and \
       all(tb_delivered(r.get("组件", "")) is not False for r in cur_fm) and \
    ...
    if not acts:
        print("(无建议——若里程碑范围有变，先更新 feature-matrix/testplan 并经 rev 复核)")
...
def main():
    parser = argparse.ArgumentParser(description="floo_axi_chimney DV 文档机械层")
