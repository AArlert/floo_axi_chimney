...
#
# 用法（本地 VM，仿真跑完后执行）：
#   python3 scripts/evidence.py --scen M1-01 --test chimney_smoke_test --seed 42
...
def extract_plain(text, log_path, scen_id):
    """非 UVM log 兜底（M0 上游 SV tb）：须有 VCS 仿真完结标记，且无 Error/Fatal 行。"""
    lines = text.splitlines()
    if not any("V C S   S i m u l a t i o n   R e p o r t" in l for l in lines):
        sys.exit(f"log 既无 UVM summary 也无 VCS 完结标记，无法判定结果: {log_path}")
    bad = [l for l in lines if re.match(r"\s*(Error|Fatal)\b", l)]
    if bad:
        sys.exit(f"log 判定 FAIL（{len(bad)} 条 Error/Fatal 行，首条: {bad[0].strip()}）"
                 "——FAIL 不登证据，去 bugs.md 登缺陷")
    idx = next(i for i, l in enumerate(lines) if "V C S   S i m u l a t i o n" in l)
...
def extract(log_path, scen_id):
    ...
    if not counts:
        return extract_plain(text, log_path, scen_id)
...
def main():
    ...
    log_path = (Path(args.log).resolve() if args.log
                else ROOT / "sim" / "out" / f"{args.test}_{args.seed}.log")
