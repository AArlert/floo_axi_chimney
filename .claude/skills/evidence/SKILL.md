...
2. 场景证据：`make evidence SCEN=<场景ID> TEST=<uvm测试> SEED=<n>`
   —— 脚本校验 UVM_ERROR/FATAL=0（非 UVM 的上游 tb log 走 VCS 完结标记+零 Error/Fatal 兜底判定）、抽取 report summary 与关键检查行、写 `doc/evidence/v<版本>/<ID>.log`（首行=复现命令）、自动回填 testplan 行（✅/证据/复现），最后跑一遍 docs-check。
